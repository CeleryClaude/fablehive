...
function start(port,htmlPath){
 ...
 const httpSrv=http.createServer((req,res)=>{
  ...
  if(u==='/'||u==='/index.html'||u==='/BROOD.html'){
   ...
   res.end(JSON.stringify({ok:1,seats:Object.keys(seats).length,souls,
    ...
    ver:'r103-the-one-framing',keeperSet:(KWH?1:0),souls:Object.keys(SOULS).length,support:(()=>{try{return _fsS.readFileSync((process.env.SUPPORT||'/opt/fablehive/support.log'),'utf8').split('\n').filter(Boolean).length;}catch(e){return 0;}})(),
 ...
 const simI=setInterval(()=>{const now=Date.now();
  ...
  if(n>0){for(const t in seats){const _w=seats[t];if(!_w||_w.readyState!==1)continue;if((_w.bufferedAmount||0)>16384)continue; /* r102 THE LIVE HAND: own-swarm hot channel - freshest own-unit truth every sim tick (60Hz), tiny, per-seat, strain-skipped (backpressure = silence, and the client falls back to the buffered path on its own). Enemies stay on the buffered 30Hz beats */
   try{const s9=G.swarms.find(z=>z.team===+t&&!z.ally);if(!s9||s9.dead)continue;_w.send(JSON.stringify({k:'o',t:now,q:[s9.x|0,s9.y|0],u:s9.units.map(u=>[u.x|0,u.y|0,u.uid||0])}));}catch(e){}}}},16); /* THE PRESSURE VALVE: a monster brawl can cost 40ms+ per tick (probe: Elder=42ms vs 8 normal). When a tick runs hot, SHED THE SIM BACKLOG, never the net beats - the meadow slows a few percent under an Elder's fury; the wire never chokes behind it */
