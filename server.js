 ...
 const httpSrv=http.createServer((req,res)=>{
  ...
  if(u==='/'||u==='/index.html'||u==='/BROOD.html'){
   ...
   res.end(JSON.stringify({ok:1,seats:Object.keys(seats).length,souls,
    ...
    ver:'r74-the-wider-calmer-view',keeperSet:(KWH?1:0),souls:Object.keys(SOULS).length,support:(()=>{try{return _fsS.readFileSync((process.env.SUPPORT||'/opt/fablehive/support.log'),'utf8').split('\n').filter(Boolean).length;}catch(e){return 0;}})(),
